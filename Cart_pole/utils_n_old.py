...
def Polytope_formation(original_polytope, hyperplane_val,Th,intersection_points):
...
def Polytope_formation_hd(original_polytope, hyperplane_val,Th,intersection_test):
    ...
    if len(intersection_test)>0:
        ...
        poly1=(original_polytope[hyperplane_val >= -1e-13])
        poly2=original_polytope[hyperplane_val <= 1e-13]
    if len(poly1)<len(original_polytope[0])+1:
...
def Enumerator_rapid(hyperplanes, b, original_polytope_test,TH,boundary_hyperplanes,border_bias,parallel):
    ...
    for i in range(len(hyperplanes)):
        ...
        for j in range(len(enumerate_poly)):
            ...
            if sgn_var[j] < -1e-10:
                if n==2:
                    intersection_points=Intersection_polytope_hype(boundary_hyperplanes,hyperplanes[i],b[i],border_bias,hyperplane_val[j],enumerate_poly[j],TH,parallel)
                    # valid_side=[]
                    ...
                    #     if np.max(hyperplane_val[j][sides[m]])>1e-10 and np.min(hyperplane_val[j][sides[m]]) <-1e-10:
                    ...
                    # if len(side)<1 and np.min(np.abs(hyperplane_val[j]))>1e-10:
                    #     raise Warning("edges are not found properly")
                    ...
                    # if len(valid_side)<n-1 and np.min(np.abs(hyperplane_val[j]))>1e-10:
                    #     raise Warning("edges are not found properly")
                    # intersection_points=finding_intersection_2D(sides1,np.array(hyperplane_val[j]),n,hyp_f,hyperplanes[i],b[i],TH,parallel)
                    # if len(intersection_points)<n-1 and np.min(np.abs(hyperplane_val[j]))>1e-10:
                    #     raise Warning("check: Number of valid sides is less than $n-1$")
                    # Calculate the new polytopes using Polytope_formation
                    original_polytope_test = Polytope_formation(enumerate_poly[j],np.array(hyperplane_val[j]),TH,intersection_points)
                    ########################
                    ...
                    # st=time.time()
                    # intersection_test=finding_intersections(sides,np.array(hyperplane_val[j]),n,hyp_f,hyperplanes[i],b[i],TH,parallel,sign_m)
                    # # end=time.time()
                    ...
                    # #         print("check")
                    # if len(intersection_test)<n-1 and np.min(np.abs(hyperplane_val[j]))>1e-10:
                    #     raise Warning("check: Number of valid sides is less than $n-1$")
                    original_polytope_test=Polytope_formation_hd(enumerate_poly[j],np.array(hyperplane_val[j]),TH,np.array(intersection_points))
...
def Intersection_polytope_hype(boundary_hyperplanes,hyperplanes,b,border_bias,hyperplane_val,enumerate_poly,TH,parallel):
    ...
    if n>2:
        ...
        sign_m=np.sign(np.dot(np.array(hyp_f)[:,0:-1],np.reshape(np.mean(enumerate_poly,axis=0),(n,1)))+np.array(hyp_f)[:,n:n+1])            
        st=time.time()
        intersection_points=finding_intersections(sides,np.array(hyperplane_val),n,hyp_f,hyperplanes,b,TH,parallel,sign_m)
        # end=time.time()
        # print("Duration:",end-st,"#combination:",math.comb(len(hyp_f),n-1))
        del sides,hyp_f
    else:
