...
def Finding_Barrier(NN_file,name_fig,eps1,eps2,TH,mode,parallel,alph,bound):
    ...
    original_polytope_test=np.array([generate_hypercube_vertices(bound)])
    cwd=os.getcwd()
    print(cwd)
    if mode=="Low_Ram":
    ...
    border_bias=[]
    for f in range(n):
        border_bias=border_bias+[TH[f]]*n
    # border_bias=[-i for i in TH*2 ]
    ...
    # all_bias=np.reshape(np.append(all_bias,np.array([-i for i in TH*2])),(len(all_hyperplanes),1))
    all_bias=np.reshape(np.append(all_bias,np.array(border_bias)),(len(all_hyperplanes),1))
    ...
    while status:
        ...
        if not status:
            ...
            a=0.1
            name="_BF_updated"
            # val=W_v@np.maximum(all_hyperplanes@np.array(zero_point).T+all_bias,0)+c_v
        ...
        iter=iter+1
    return Original_NN,BF_NN,all_hyperplanes,all_bias,W,c,enumerate_poly,D,border_hyperplane,border_bias,np.unique(zero_point,axis=0),W_v,c_v
