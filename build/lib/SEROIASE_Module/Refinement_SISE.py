...
def Refinement(enumerate_poly,all_hyperplanes,all_bias,slack_var,sol,W,c,eps,D,boundary_regions,tau_b,tau_agis,AGIS_region,tau_nagis,NAGIS_region):
    n_r=len(enumerate_poly)
    n_h,n=np.shape(all_hyperplanes)
    W_v=sol[0:n_h]
    W_v=np.reshape(W_v,(1,n_h))
        ...
        for i in range(len(tau_agis)):
            if tau_agis[i]>=eps:
                ...
                for j in reg:
                    ...
                    x_dot_m=np.sum(x_dot,axis=1,keepdims=True)/len(x_dot)
                    mid_point=np.mean(enumerate_poly[j],axis=0,keepdims=True)
                    h_new= x_dot_m.T 
                    b_new=-h_new@mid_point.T 
                    regs.append(j)    
                    hyperplanes.extend(h_new)
                    ...
                    bias.extend(b_new[0])
                    all_bias_new=np.vstack((all_bias_new,b_new,-b_new))
    # if np.max(tau_nagis)>=eps:
    #     for i in range(len(tau_nagis)):
    #         if slack_var[i]>=eps:
    #             reg=NAGIS_region[i]
    #             for j in reg:
    #                 x_dot=np.dot(W,np.maximum(np.dot(all_hyperplanes,np.array(enumerate_poly[j]).T)+all_bias,0))+c
    ...
    #                 all_hyperplanes_new=np.vstack((all_hyperplanes_new,h_new,-h_new))
    #                 bias.extend(b_new[0])
    #                 all_bias_new=np.vstack((all_bias_new,b_new))
